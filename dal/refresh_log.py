...
def create_refresh_event(
    conn: sqlite3.Connection, run_id: str, institution_id: str, state: str = "STARTED"
) -> int:
...
def get_run_events(conn: sqlite3.Connection, run_id: str) -> list[dict]:
    """Get all events for a given refresh run."""
    rows = conn.execute(
        "SELECT * FROM refresh_events WHERE run_id = ? ORDER BY created_at ASC",
        (run_id,),
    ).fetchall()
    return [dict(r) for r in rows]


def get_refresh_history(conn: sqlite3.Connection, limit: int = 20) -> list[dict]:
