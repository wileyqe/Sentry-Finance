...
def _resolve_account(institution: str, account: str) -> tuple[str, str]:
    ...
    if not account_id:
        ...
        log.warning(
            "Could not resolve account for %s/%s, using fallback: %s",
            institution,
            account,
            account_id,
        )
...
def migrate_csv_file(csv_path: pathlib.Path, conn, dry_run: bool = False) -> dict:
    ...
    if df.empty:
        ...
        return {"file": csv_path.name, "rows": 0, "inserted": 0, "updated": 0}

    # Detect institution/account from CSV content
    institution = (
        df["institution"].iloc[0] if "institution" in df.columns else "Unknown"
    )
    ...
    for _, row in df.iterrows():
        ...
        txn_date = (
            str(row.get("txn_date", ""))[:10] if pd.notna(row.get("txn_date")) else None
        )
    ...
    log.info(
        "  ✔  %s: %d rows → %d inserted, %d updated, %d unchanged",
        csv_path.name,
        len(txns),
        stats["inserted"],
        stats["updated"],
...
def migrate_all(dry_run: bool = False) -> list[dict]:
    ...
    with get_db() as conn:
        for csv_path in csv_files:
            try:
                result = migrate_csv_file(csv_path, conn, dry_run=dry_run)
                results.append(result)
            except Exception as e:
                log.error("  ✗  Failed to migrate %s: %s", csv_path.name, e)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Migrate CSV data into SQLite")
    parser.add_argument(
