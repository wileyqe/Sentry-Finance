...
Matt Pocock's 12 installed skills (`tdd`, `diagnose`, `grill-me`,
`grill-with-docs`, `triage`, `to-issues`, `to-prd`,
`improve-codebase-architecture`, `zoom-out`, `caveman`, `write-a-skill`,
`setup-matt-pocock-skills`) are explicit user-invoked workflow commands from
`~/.claude/skills/` and `~/.codex/skills/`. Do not auto-activate them, and do
not duplicate them as Sentry-local skills.
