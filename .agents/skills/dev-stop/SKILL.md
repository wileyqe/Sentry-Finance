...
Important: the Node check must EXCLUDE the Codex MCP extension processes
under `AppData\Roaming\Codex\Codex Extensions\` — those are unrelated and
killing them breaks the current Codex session. The regex above scopes the
