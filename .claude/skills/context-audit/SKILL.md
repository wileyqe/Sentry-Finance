---
name: context-audit
description: Audit Claude Code config for token waste — CLAUDE.md, allow-list, memory, skills, settings, deny rules. Scores health and offers fixes.
user-invocable: true
---

# Context Health Audit

Find token waste and context bloat in this project's Claude Code configuration.
...
```
# Project CLAUDE.md
C:/Users/chang/OneDrive/Desktop/Projects/Personal Finance Project/CLAUDE.md

# Settings (allow-list, deny rules)
C:/Users/chang/OneDrive/Desktop/Projects/Personal Finance Project/.claude/settings.local.json

# Memory index and individual memory files
C:/Users/chang/.claude/projects/C--Users-chang-OneDrive-Desktop-Projects-Personal-Finance-Project/memory/MEMORY.md
# Then glob: C:/Users/chang/.claude/projects/C--Users-chang-OneDrive-Desktop-Projects-Personal-Finance-Project/memory/*.md
...
Record line counts for CLAUDE.md, each skill, and the memory index. Record
...
**Five-filter rule scan.** Read every rule and instruction in CLAUDE.md.
Flag any that match:

| Filter        | Flag when...                                                        |
|---------------|---------------------------------------------------------------------|
| Default       | Claude already does this without being told ("write clean code")    |
...
Parse `permissions.allow` from `.claude/settings.local.json`.
...
correct has been codified into CLAUDE.md.

**Scoring (max -10):**

| Condition                        | Deduction |
|----------------------------------|-----------|
| Memory duplicates CLAUDE.md      | -3 each   |
...
project level in `.claude/settings.local.json` that duplicate CLI
functionality.
...
|------------------|-----------|-----|-----------------------|
| CLAUDE.md        | -{n}      | -15 | {line count, flags}   |
...
### Flagged Rules
{Each flagged rule from CLAUDE.md or skills: the text, which filter, why}
...
- CLAUDE.md rule removal: show each flagged rule with its filter and reason
