...
- Fidelity account `fidelity_0827` exists in DB, type=investment, zero data
...
- Clear `investment_holdings` for `fidelity_0827` before re-seeding (idempotent)
...
4. `curl /api/investments/lots?account_id=fidelity_0827&ticker=AAPL` returns lot detail
